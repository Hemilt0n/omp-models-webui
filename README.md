...
- [Bun](https://bun.sh/) 1.3.14 或更新版本；
- [OMP](https://omp.sh/) 16 或更新版本；
...
安装后重启 OMP，使用 `/models-ui` 即可发现命令。运行 `omp plugin list` 确认插件已安装并启用；若加载失败，再运行 `omp plugin doctor` 检查插件清单和依赖。
